...
export default async function TableFlete() {
    ...
    return (
        <>
        <div className="col-span-12 rounded-full bg-zinc-800 items-center justify-items-center gap-12 text-center font-[family-name:var(--font-geist-sans)]">
            ...
            <tbody>
                {fletes && fletes.map((flete: any, index: number) => (
                    <tr className={index % 2 ? "bg-stone-600 text-sm" : "text-sm"} key={flete.id}>
                        ...
                        <td>$<input name='costo' id='costo' readOnly className='text-white text-center h-2 bg-zinc-800 rounded-full'  value={flete.costo}/></td>
