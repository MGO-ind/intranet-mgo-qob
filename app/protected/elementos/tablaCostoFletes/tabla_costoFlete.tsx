                ...
                {fletes && fletes.map((flete: any, index: number) => (
                    <tr className={index % 2 ? "bg-stone-600 text-sm" : "text-sm"} key={flete.id}>
                        ...
                        <td className='mx-2 my-2'>
                            ...
                            <button className="rounded-full border-green-300 border  transition-colors hover:bg-green-500 dark:bg-green-700 text-sm sm:text-base h-8 w-32 sm:h-7 px-2 sm:px-5 m-2" onClick={() => navigator.clipboard.writeText(flete.costo)}>Copiar</button>
